...
const SPECIALS = {
  ...
  // ---- ACT TWO — creatures of the eras ----
  raptor:         { name: 'Sickle Pounce',  kind: 'sword',   behavior: 'shot',  color: '#7a9a44', speed: 2.1 },
  stego:          { name: 'Tail Spikes',    kind: 'bone',    behavior: 'shot',  color: '#5f8060', speed: 1.6, count: 2 },
  ptero:          { name: 'Dive Shriek',    kind: 'howl',    behavior: 'shot',  color: '#9a6a44', speed: 2.0 },
  outlaw:         { name: 'Six-Shooter',    kind: 'beam',    behavior: 'shot',  color: '#e8d9a0', speed: 2.3 },
  grandpa:        { name: 'Cane Fling',     kind: 'axe',     behavior: 'shot',  color: '#8a6a4a', speed: 1.5 },
  coyote:         { name: 'Yowl',           kind: 'howl',    behavior: 'shot',  color: '#b89a6a', speed: 1.8 },
  soldier:        { name: 'Rifle Burst',    kind: 'beam',    behavior: 'shot',  color: '#c8d0b8', speed: 2.3, count: 2 },
  drone:          { name: 'Gun Strafe',     kind: 'beam',    behavior: 'shot',  color: '#ff6a5a', speed: 2.4 },
  jet:            { name: 'Missile',        kind: 'cannon',  behavior: 'lob',   color: '#5a6470', speed: 1.2 },
  scarab:         { name: 'Shell Ricochet', kind: 'rock',    behavior: 'shot',  color: '#2a7a6a', speed: 1.9 },
  jackal:         { name: 'Khopesh Arc',    kind: 'sword',   behavior: 'shot',  color: '#c9a24a', speed: 2.0 },
  merman:         { name: 'Trident Jet',    kind: 'water',   behavior: 'shot',  color: '#4a9a8a', speed: 2.1 },
  jellyfish:      { name: 'Sting Cloud',    kind: 'venom',   behavior: 'slick', color: '#c98ad9', speed: 1.2 },
  gladiator:      { name: 'Gladius Throw',  kind: 'sword',   behavior: 'shot',  color: '#c98a5a', speed: 1.9 },
  centurion:      { name: 'Pilum',          kind: 'bone',    behavior: 'shot',  color: '#b8342a', speed: 2.0 },
  caveman:        { name: 'Club Lob',       kind: 'rock',    behavior: 'lob',   color: '#8a5a30', speed: 1 },
  sabertooth:     { name: 'Fang Leap',      kind: 'sword',   behavior: 'shot',  color: '#d9a45a', speed: 2.2 },
  amoeba:         { name: 'Blob Split',     kind: 'goo',     behavior: 'slick', color: '#7ad9a0', speed: 1.3 },
  trilobite:      { name: 'Shell Spin',     kind: 'rock',    behavior: 'shot',  color: '#8a6a4a', speed: 1.8 },
  robot:          { name: 'Eye Laser',      kind: 'beam',    behavior: 'shot',  color: '#30ffb0', speed: 2.4 },
  mech:           { name: 'Rocket Pod',     kind: 'cannon',  behavior: 'lob',   color: '#ff4a6a', speed: 1.2 },
  // ---- ACT THREE — the futuristic rebuilds (lasers for everyone) ----
  cyber_zombie:   { name: 'Laser Eye',      kind: 'beam',    behavior: 'shot',  color: '#4ae0c8', speed: 2.2 },
  void_squid:     { name: 'Plasma Ink',     kind: 'ink',     behavior: 'slick', color: '#7a5aff', speed: 1.4 },
  laser_swordfish:{ name: 'Cutting Beam',   kind: 'beam',    behavior: 'shot',  color: '#4ab8ff', speed: 2.5 },
  cryo_yeti:      { name: 'Nitrogen Breath',kind: 'ice',     behavior: 'shot',  color: '#bff0ff', speed: 2.0 },
  hover_bear:     { name: 'Hover Slam',     kind: 'shock',   behavior: 'lob',   color: '#8ac0e8', speed: 1.1 },
  frost_drone:    { name: 'Freeze Ray',     kind: 'ice',     behavior: 'shot',  color: '#c8e8ff', speed: 2.3 },
  laser_pup:      { name: 'Laser Eyes',     kind: 'beam',    behavior: 'shot',  color: '#c04aff', speed: 2.2, count: 2 },
  ring_phantom:   { name: 'Ring Shard',     kind: 'ghost',   behavior: 'shot',  color: '#e8d8ff', speed: 2.0 },
  jet_werewolf:   { name: 'Afterburner Howl',kind: 'howl',   behavior: 'shot',  color: '#8a80c9', speed: 2.3 },
  storm_amoeba:   { name: 'Lightning Blob', kind: 'goo',     behavior: 'slick', color: '#ffd24a', speed: 1.4 },
  plasma_jelly:   { name: 'Plasma Sting',   kind: 'venom',   behavior: 'slick', color: '#ff8ae8', speed: 1.3 },
  thunder_mech:   { name: 'Thunder Bolt',   kind: 'beam',    behavior: 'shot',  color: '#ffd24a', speed: 2.5 },
  rust_raptor:    { name: 'Rocket Claw',    kind: 'sword',   behavior: 'shot',  color: '#d96a3a', speed: 2.3 },
  war_drone:      { name: 'Twin Lasers',    kind: 'beam',    behavior: 'shot',  color: '#d94a3a', speed: 2.4, count: 2 },
  dust_soldier:   { name: 'Rail Shot',      kind: 'beam',    behavior: 'shot',  color: '#c97a4a', speed: 2.6 },
  chrome_zombie:  { name: 'Chrome Spit',    kind: 'goo',     behavior: 'slick', color: '#b8c8d8', speed: 1.4 },
  hunter_mech:    { name: 'Seeker Rocket',  kind: 'cannon',  behavior: 'lob',   color: '#ff4a6a', speed: 1.3 },
  security_bot:   { name: 'Stun Laser',     kind: 'beam',    behavior: 'shot',  color: '#30ffb0', speed: 2.3 },
  acid_gooster:   { name: 'Acid Sling',     kind: 'venom',   behavior: 'slick', color: '#c9ff4a', speed: 1.4 },
  inferno_scarab: { name: 'Ember Shell',    kind: 'fire',    behavior: 'lob',   color: '#ff8a3a', speed: 1.2 },
  lava_trilobite: { name: 'Magma Spin',     kind: 'fire',    behavior: 'shot',  color: '#ff6a4a', speed: 1.9 },
  solar_skeleton: { name: 'Solar Flare',    kind: 'fire',    behavior: 'shot',  color: '#ffe08a', speed: 2.2 },
  mercury_crab:   { name: 'Mercury Snip',   kind: 'water',   behavior: 'shot',  color: '#e8e8f0', speed: 2.1 },
  quick_wraith:   { name: 'Silver Bolt',    kind: 'ghost',   behavior: 'shot',  color: '#e8f0ff', speed: 2.7 },
  // ---- ACT TWO & THREE wardens (arena boss cards) ----
  trex:           { name: 'Tyrant Roar',    kind: 'howl',    behavior: 'shot',  color: '#6a8040', speed: 2.0, count: 2, dmgMul: 1.2 },
  iron_horse:     { name: 'Smokestack Blast',kind: 'cannon', behavior: 'lob',   color: '#5a5a66', speed: 1.2, dmgMul: 1.2 },
  warhound:       { name: 'Tank Shell',     kind: 'cannon',  behavior: 'lob',   color: '#5a6650', speed: 1.3, dmgMul: 1.25, radius: 0.2 },
  anubis:         { name: 'Judgement Scale',kind: 'skull',   behavior: 'shot',  color: '#d4af37', speed: 2.1, count: 2, dmgMul: 1.2 },
  kraken:         { name: 'Tentacle Slam',  kind: 'water',   behavior: 'lob',   color: '#6a3a7a', speed: 1.2, dmgMul: 1.25, radius: 0.2 },
  colossus:       { name: 'Molten Fist',    kind: 'fire',    behavior: 'lob',   color: '#ff6a2a', speed: 1.1, dmgMul: 1.25, radius: 0.22 },
  mammoth_king:   { name: 'Tusk Avalanche', kind: 'snow',    behavior: 'lob',   color: '#e8e0d0', speed: 1.1, dmgMul: 1.2, radius: 0.22 },
  magma_worm:     { name: 'Magma Spray',    kind: 'fire',    behavior: 'slick', color: '#ff7a2a', speed: 1.3, dmgMul: 1.2 },
  backstabber_prime:{ name: 'Prime Daggers',kind: 'shadow',  behavior: 'shot',  color: '#30ffb0', speed: 2.4, count: 4, dmgMul: 1.2 },
  mecha_shark:    { name: 'Fin Lasers',     kind: 'beam',    behavior: 'shot',  color: '#4ae0ff', speed: 2.5, count: 2, dmgMul: 1.2 },
  cryo_titan:     { name: 'Freeze-Ray Breath',kind: 'ice',   behavior: 'shot',  color: '#dff4ff', speed: 2.2, count: 2, dmgMul: 1.2 },
  laser_alpha:    { name: 'Laser Eye Sweep',kind: 'beam',    behavior: 'shot',  color: '#b04aff', speed: 2.5, count: 3, dmgMul: 1.2 },
  storm_colossus: { name: 'Thunderclap',    kind: 'shock',   behavior: 'lob',   color: '#4ad9ff', speed: 1.2, dmgMul: 1.25, radius: 0.22 },
  war_machine:    { name: 'Twin Cannons',   kind: 'cannon',  behavior: 'lob',   color: '#ff6a3a', speed: 1.3, dmgMul: 1.25, radius: 0.2 },
  maglev:         { name: 'Rail Surge',     kind: 'beam',    behavior: 'shot',  color: '#30ffb0', speed: 2.7, dmgMul: 1.25 },
  plasma_leviathan:{ name: 'Plasma Geyser', kind: 'fire',    behavior: 'slick', color: '#ffe04a', speed: 1.4, dmgMul: 1.25 },
  quicksilver:    { name: 'Silver Storm',   kind: 'ghost',   behavior: 'shot',  color: '#eef2fa', speed: 3.0, count: 3, dmgMul: 1.2 },
  backstabber_omega:{ name: 'Omega Barrage',kind: 'shadow',  behavior: 'shot',  color: '#ff2d5a', speed: 2.6, count: 5, dmgMul: 1.3 },
};
